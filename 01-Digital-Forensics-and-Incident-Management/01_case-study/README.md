# Case Study
This folder contains the case study report, presentation and contemporaneous note.

## Chain of Custody (Academic Simulation)

- Evidence acquired using write-blocking and forensic imaging tools
- Hashes generated to verify integrity
- Original evidence not modified
- All published artefacts are redacted derivatives

## Incident Management Considerations

From an incident management perspective, the case highlights:

- Early behavioural warning indicators
- Importance of timely intervention
- Role of digital evidence in threat assessment
- Need for multidisciplinary response (forensics, legal, safeguarding)

## Academic Integrity Statement

This repository represents original academic work submitted for educational
purposes. All sources are cited where applicable. Redactions were applied
to ensure ethical and lawful disclosure.


## Future Work

If extended beyond the scope of this coursework, future work could include:

- Memory analysis for volatile artefacts
- Network traffic reconstruction
- Correlation with external OSINT sources
- Formal threat-assessment framework application


